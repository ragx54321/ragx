...
- **Alternative Algorithms - IVF**
  - We have also included scripts to setup Inverted Vector Files. Inverted Vector Files can be used to replace an HNSW. Please refer to FAISS's discussion on what type of index to choose - https://github.com/facebookresearch/faiss/wiki/Faiss-indexes.

- **Alternative Algorithms - ScANN**
  - We have also included scripts to setup ScANN. Please refer to ScANN's documentation - https://github.com/google-research/google-research/tree/master/scann.
...
| BM25 - Inverted Index Construction Time   | < 10 minutes (< $1)| < 30 minutes (< $3)| < 1 hour (< $6)    | TBD                 |
...
| Doc2Vec - HNSW Construction Time          | < 15 minutes (< $1.5)| < 1 hour (< $6) | 4 hours ($24)      | TBD                 |
| GTR - Embedding Time                      | < 1 hour (< $6)    | 16 hours (96)     | 72 hours ($432)    | TBD                 |
| GTR - HNSW Construction Time              | < 30 minutes (< $3)| < 1 hour (< $6)   | 6 hours ($36)      | TBD                 |
...
|                                         | 0.5 Million Passages | 5 Million Passages | 50 Million Passages | 500 Million Passages |
|-----------------------------------------|----------------------|--------------------|---------------------|----------------------|
| BM25 - Dataset Size (Tokenized Words)   | .26 GB               | 3.2 GB             | 32 GB               |          TBD         |
| SPLADEv2 - Dataset Size (Embeddings)    | 1.5 GB               | 15 GB              | 150 GB              |          1500 GB     |
...
cd src_retrievers
cd [desired_benchmark] # Replace ColBERT with desired benchmark
vim embed_corpus.py
# Set desired paths/parameters within embed_corpus file
python3 embed_corpus.py 

# Collect data for any benchmark.
cd src_retrievers/[desired_benchmark]

# Set desired paths/parameters within *_collect file
...
- `retrievers/`: Contains code for implementing and testing five retrieval models (BM25, SPLADEv2, ColBERT, Doc2Vec, GTR).
- `llm_backend/`: Houses the LLM backend code for integrating LLAMA2 and other models using Hugging Face and TensorRT.
- `vector_databases/`: Contains implementations for various vector databases like FAISS (HNSW, IVF) and ScANN for efficient retrieval.
- `datasets/`: Includes scripts for downloading and processing datasets like PubMed, Wikipedia, and BioASQ for use in benchmarking.
- `queries/`: Contains example queries to use for testing and benchmarking different RAG configurations.
