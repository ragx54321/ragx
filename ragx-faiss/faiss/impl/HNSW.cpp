...
FILE* embedding_file = fopen("embeddings.bin", "rb"); // Open the file globally if needed
...
int search_from_candidates(
    ...
    while (candidates.size() > 0) {
        ...
        for (size_t j = begin; j < jmax; j++) {
            ...
            // Read `v1 * 128 * 4` bytes from the file
            std::vector<float> buffer(128); // Buffer for one vector (128 floats)
            size_t offset = static_cast<size_t>(v1) * 128 * sizeof(float);

            fseek(embedding_file, offset, SEEK_SET);
            fread(buffer.data(), sizeof(float), 128, embedding_file);
