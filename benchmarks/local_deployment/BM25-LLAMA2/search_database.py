...
# Function to run BM25 retrieval and collect timings
def run_bm25_retriever(index_path, queries_file, k=100, iterations=1, max_queries=None, num_queries_with_distinct_count=0, max_docs=None, dump_postings=False):
    ...
    for query_idx, query_data in enumerate(queries):  # Iterate over each query
        ...
        # Collect timing data
        timings.append({
            'query_processing_times': query_processing_times,
            'search_times': search_times,
            'scoring_times': scoring_times,
            'num_documents_scored': num_documents_scored
        })
        ...
        print(f"  Distinct Documents Scored: {np.mean(num_documents_scored):.0f}")
        print("-" * 40)


if __name__ == "__main__":
    queries_file = 'INSERT QUERIES PATH HERE'
    index_path = 'INSERT INDEX PATH HERE'
    run_bm25_retriever(index_path=index_path, queries_file=queries_file, k=100, iterations=1, max_queries=10)