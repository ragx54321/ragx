...
if __name__ == "__main__":
    run_experiment(
        queries_file='INSERT QUERY PATH',
        output_csv='OUTPUT CSV PATH',
        corpus_file='PUBMED/OTHER DATASET PATH',
        num_queries=1,
        topK=100,
        iterations=1,  # Set the number of iterations for timing
        max_docs=0,  # Limit the number of documents loaded from the corpus
        collect=['all'],  # Options: 'embedding', 'search', 'topK', 'all'
        num_threads=16,  # Restrict the number of threads for Gensim inference and FAISS
        model_path="DOC2VEC MODEL PATH",  # Set the path for saving/loading the model
        hnsw_path="HNSW FILE PATH"  # Set the path for saving/loading the HNSW index
    )
