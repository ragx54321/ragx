...
def main(corpus_file, query_file, num_docs=100000, num_queries=1000, k=10, index_save_file="", save_hdf5=False, hdf5_file=""):
    ...
    query_embeddings = []
    batch_size = 32  # Reduced batch size due to T5's larger model size
...
if __name__ == "__main__":
    ...
    main(
        ...
        query_file='path to queries',
        num_docs='set number of passages to embed',
        num_queries=1,  # Does not matter for embedding
        k='set top-K', # Does not matter for embedding
        index_save_file="path to index",  # Specify the file name for the index
        save_hdf5=False,  # Set to True to save embeddings as HDF5, False to save as FAISS index
        hdf5_file="path to hdf5"  # File to save HDF5 embeddings
    )