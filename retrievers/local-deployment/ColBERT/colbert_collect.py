...
def process_query(query, model, tokenizer, faiss_index, batch_size, iterations, num_threads):
    ...
    for _ in range(iterations):
        ...
        start_time = time.time()
        faiss_index.search(embeddings[-1:], 1000)
        end_time = time.time()
        #print(query_batch)
        #print(embeddings[-1:])
...
if __name__ == "__main__":
    run_bert_retriever(
        queries_file='INSERT QUERIES PATH',
        output_csv='query_timing_data.csv',
        index_path='INSERT FAISS HNSW PATH',
        num_queries=1,  # Set the number of queries to process for testing
        batch_size=1,  # Set the batch size for each query
        iterations=1,  # Set the number of iterations to repeat for timing
        num_threads=32  # Set number of threads for embedding
    )
